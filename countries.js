...
function sendNewState(){
    ...
    let newStateCountryId = dropdown.options[dropdown.selectedIndex].getAttribute('data-countryid');
...
/*
    Event Listeners
 */
document.getElementById("submit-country").addEventListener("click", function(event){
    ...
    else{ alert("All fields are required to submit.");}

  });

document.getElementById("submit-state").addEventListener("click", function(event){
    event.preventDefault();
    ...
    if (validateInput(newStateName) && validateInput(newStateCode) &&  validateInput(newStateCountry)){
        sendNewState();
        form.reset();
    }
    else{ alert("All fields are required to submit.");}
